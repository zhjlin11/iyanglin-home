...
import { backupForumProdDb } from "./backup-forum-prod-db";
...
export async function importForumProduction() {
  console.log("🚀 正在执行 第二阶段：Forum 社区论坛正式生产数据库 (yanglin_db) 迁移...\n");

  const startTime = Date.now();

  // 1. Perform pre-migration snapshot backup
  const backupResult = await backupForumProdDb();
  const preMigrationCount = await prisma.post.count();
  ...
  for (const item of normalizedList) {
    try {
      ...
      errorCount++;
      console.error(`❌ 社区帖子 ${item.oldId} 迁移失败:`, err);
    }
  }

  const durationMs = Date.now() - startTime;
  const postMigrationCount = await prisma.post.count();
...
if (require.main === module) {
  importForumProduction().catch(console.error);
}
