...
export async function backupEventProdDb() {
  console.log("🛡️ 正在执行 第一阶段：Event 生产迁移前全量数据库备份...\n");

  const backupDir = "/var/backups/iyanglin/event-migration";
  if (!fs.existsSync(backupDir)) {
    fs.mkdirSync(backupDir, { recursive: true });
  }

  const timestamp = Date.now();
  const backupFile = path.join(backupDir, `yanglin_db_before_event_prod_001_${timestamp}.json`);
  ...
  const payload = {
    metadata: {
      ...
      target: "yanglin_db",
      phase: "Phase 34 Event Production Migration",
      counts: {
        users: users.length,
        jobs: jobs.length,
  ...
  console.log(`- 备份前 Event 记录数: ${events.length}`);
  console.log("--------------------------------------------------\n");

  await prisma.$disconnect();
  return { backupFile, stat, hash, initialEventCount: events.length };
}

if (require.main === module) {
  backupEventProdDb().catch(console.error);
}
