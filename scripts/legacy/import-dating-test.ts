...
export async function runTestDbDatingVerification() {
  console.log("🧪 正在执行 Stage 6: 隔离测试数据库 (yanglin_migration_test) 导入、幂等性与回滚验证...\n");

  // 1. Check production database initial count
  const initialProdCount = await prodPrisma.datingProfile.count();
  console.log(`[生产保护确认] 生产数据库 yanglin_db 当前 DatingProfile 记录数: ${initialProdCount}`);

  // 2. Perform Dry Run to get normalized records
  const { normalizedList } = await runDatingDryRun();

  // 3. Clear isolated test database DatingProfile table for clean start
  await testPrisma.datingProfile.deleteMany({});
  const initialTestCount = await testPrisma.datingProfile.count();
  console.log(`[隔离测试确认] 测试数据库 yanglin_migration_test 初始 DatingProfile 记录数: ${initialTestCount}`);

  // 4. Test DB Insert (Batch 1)
  console.log("\n【步骤 1: 向测试数据库写入样本 Dating 数据】");
  let insertedCount = 0;
  for (const item of normalizedList) {
    await testPrisma.datingProfile.create({
      data: {
  ...
  const afterInsertTestCount = await testPrisma.datingProfile.count();
  console.log(`- 成功向测试数据库插入条数: ${insertedCount}`);
  console.log(`- 测试数据库当前 DatingProfile 记录数: ${afterInsertTestCount}`);
  ...
  for (const item of normalizedList) {
    const existing = await testPrisma.datingProfile.findUnique({ where: { oldId: item.oldId } });
    if (existing) {
      skippedCount++;
    } else {
      await testPrisma.datingProfile.create({
  ...
  const afterRollbackTestCount = await testPrisma.datingProfile.count();
  console.log(`- 回滚后测试数据库 DatingProfile 记录数: ${afterRollbackTestCount}`);
  console.log(`- 事务回滚校验结果: ${afterRollbackTestCount === 0 ? "✅ 100% 回滚成功" : "❌ 回滚失败"}`);

  // 7. Verify zero change on production database
  const finalProdCount = await prodPrisma.datingProfile.count();
  console.log("\n==================================================");
  console.log(`[生产零变化确认] 生产数据库 yanglin_db 最终 DatingProfile 记录数: ${finalProdCount}`);
...
if (require.main === module) {
  runTestDbDatingVerification().catch(console.error);
}
