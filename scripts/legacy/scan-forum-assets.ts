...
export function scanForumAssets() {
  console.log("🔍 正在执行 第一阶段：社区论坛 (Forum / BBS / 贴吧/讨论) 历史资产深度扫描...\n");
  ...
  const forumFiles: Array<{ path: string; size: number; encoding: string; fileType: string }> = [];
  ...
  console.log("=== 社区论坛历史资产扫描清单 ===");
  console.log(`- 共找到 ${forumFiles.length} 个相关离线资产文件:`);
  forumFiles.forEach((f, idx) => {
