...
import { backupHouseProdDb } from "./backup-house-prod-db";
...
export async function importHouseProduction() {
  console.log("🚀 正在执行 第二部分：House 房产楼市正式生产数据库 (yanglin_db) 迁移...\n");

  const startTime = Date.now();

  // 1. Perform pre-migration snapshot backup
  const backupResult = await backupHouseProdDb();
  const preMigrationCount = await prisma.house.count();
  ...
  for (const item of normalizedList) {
    try {
      ...
      errorCount++;
      console.error(`❌ 房源记录 ${item.oldId} 迁移失败:`, err);
    }
  }

  const durationMs = Date.now() - startTime;
  const postMigrationCount = await prisma.house.count();

  console.log("=== 第二部分：正式生产迁移统计结果 ===");
...
if (require.main === module) {
  importHouseProduction().catch(console.error);
}
