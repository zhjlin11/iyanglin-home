...
export async function backupShopProdDb() {
  console.log("🛡️ 正在执行 第一阶段：Shop 生产迁移前全量数据库备份...\n");

  const backupDir = "/var/backups/iyanglin/shop-migration";
  if (!fs.existsSync(backupDir)) {
    fs.mkdirSync(backupDir, { recursive: true });
  }

  const timestamp = Date.now();
  const backupFile = path.join(backupDir, `yanglin_db_before_shop_prod_001_${timestamp}.json`);
  ...
  const payload = {
    metadata: {
      ...
      target: "yanglin_db",
      phase: "Phase 32 Shop Production Migration",
      counts: {
        users: users.length,
        jobs: jobs.length,
  ...
  console.log(`- 备份前 Shop 记录数: ${shops.length}`);
  console.log("--------------------------------------------------\n");

  await prisma.$disconnect();
  return { backupFile, stat, hash, initialShopCount: shops.length };
}

if (require.main === module) {
  backupShopProdDb().catch(console.error);
}
